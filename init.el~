...
(use-package evil-collection
  ...
  ;;(evil-define-key 'visual 'global (kbd "J") (lambda () (interactive) (evil-next-line 6)))
  ;;(evil-define-key 'visual 'global (kbd "K") (lambda () (interactive) (evil-previous-line 6)))
...
(use-package pdf-tools
  ...
  ;; Configuración de "Midnight Mode" (Modo oscuro / Invertir colores)
  ;; Podés cambiar estos hex codes por los de tu colorscheme de Neovim
  ;;(setq pdf-view-midnight-colors '("#ebdbb2" . "#282828")) esto usaba en gruvbox
  (setq pdf-view-midnight-colors '("#d8dee9" . "#1b2b34"));;doom-oceanic-next
  ;; Limpiar la interfaz al abrir un PDF
  (add-hook 'pdf-view-mode-hook
            (lambda ()
              (pdf-view-midnight-minor-mode 1) ; Arrancar siempre con colores invertidos
              (display-line-numbers-mode -1)   ; Apagar números de línea
              (blink-cursor-mode -1)           ; Apagar cursor
              (auto-revert-mode 1)))           ; Recargar automáticamente si el PDF cambia
...
(defun my-eshell-directory-at-point ()
  "Devuelve el directorio bajo el cursor, o nil."
...
(defun my-eshell-smart-return ()
  "En eshell:
- sobre un directorio -> cd
- en el prompt -> enviar comando
- en cualquier otro lugar -> comportamiento normal."
  (interactive)
  (if-let ((dir (my-eshell-directory-at-point)))
      (progn
        (goto-char (point-max))
        (insert (concat "cd " (shell-quote-argument dir) " && ls -d */"))
        (eshell-send-input)
        ;; mejorar esto (en vez de 4 lineas buscar el comienzo de ls)
        (forward-line -3)
      )
    (if (>= (point) (marker-position eshell-last-output-end))
        (eshell-send-input)
      (goto-char (point-max)))))
...
(with-eval-after-load 'evil
  (add-hook 'eshell-mode-hook
            (lambda ()
              (evil-define-key 'normal eshell-mode-map (kbd "RET") #'my-eshell-smart-return)
              (evil-define-key 'normal eshell-mode-map (kbd "-")
                (lambda ()
                  (interactive)
                  (goto-char (point-max))
                  (insert ".. && ls -d */")
...
(with-eval-after-load 'ivy
  ...
  (define-key ivy-reverse-i-search-map (kbd "C-k") 'ivy-previous-line)
  (define-key ivy-reverse-i-search-map (kbd "C-l") 'ivy-call)
  (define-key ivy-switch-buffer-map (kbd "C-l") 'ivy-call)
  ...
  (define-key ivy-reverse-i-search-map (kbd "K") 'ivy-previous-line)
  (define-key ivy-reverse-i-search-map (kbd ":") 'ivy-call)
  (define-key ivy-switch-buffer-map (kbd ":") 'ivy-call)
  (define-key ivy-minibuffer-map (kbd ":") 'ivy-call)
  )
...
(use-package doom-modeline
  ...
  :config
    ...
    (display-time-mode 1)
    (setq display-time-format " 󰥔 %I:%M %p ");; este no esta mal pero el de abajo tiene los dias en espanol
...
(setq doom-modeline-buffer-file-name-style 'truncate-upto-project) ;; es un poco menos minimalista en vez del nombre solo pero esta bueno
...
(custom-set-variables
 ...
 '(package-selected-packages
   '(year-1984-theme dired-ranger ivy-prescient nerd-icons doom-modeline ivy-rich counsel ivy wolfram pyvenv doom-themes image-mode org-download yasnippets calfw-component evil-leader evil-escape dirvish))
